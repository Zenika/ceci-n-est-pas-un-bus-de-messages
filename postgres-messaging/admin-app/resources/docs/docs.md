...
The `home-routes` handler in the `admin-app.routes.home` namespace
...
The `home-page` function will in turn call the `admin-app.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `admin-app.handler` namespace:
...
Request middleware functions are located under the `admin-app.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `admin-app.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `admin-app.db.core` in some other namespace.
